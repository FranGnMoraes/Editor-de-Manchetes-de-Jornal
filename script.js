    ...
    function renderCardEditors() {
        ...
        state.cards.forEach((card, index) => {
            ...
            editorEl.innerHTML = `
                ...
                <div class="mb-2"><label for="card-subtitle-${card.id}" class="text-sm font-medium">Subtítulo</label><textarea id="card-subtitle-${card.id}" class="card-subtitle w-full p-2 border border-gray-300 rounded-md mt-1" rows="3">${card.subtitle}</textarea></div>
                
                <!-- Controles de checkbox para negrito e itálico -->
                <div class="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-2 mt-4 mb-4">
                    <label for="card-title-bold-${card.id}" class="flex items-center space-x-2 text-sm cursor-pointer"><input type="checkbox" id="card-title-bold-${card.id}" class="card-title-bold" ${card.isTitleBold ? 'checked' : ''}><span>Título Negrito</span></label>
                    <label for="card-title-italic-${card.id}" class="flex items-center space-x-2 text-sm cursor-pointer"><input type="checkbox" id="card-title-italic-${card.id}" class="card-title-italic" ${card.isTitleItalic ? 'checked' : ''}><span>Título Itálico</span></label>
                    <label for="card-subtitle-bold-${card.id}" class="flex items-center space-x-2 text-sm cursor-pointer"><input type="checkbox" id="card-subtitle-bold-${card.id}" class="card-subtitle-bold" ${card.isSubtitleBold ? 'checked' : ''}><span>Subtítulo Negrito</span></label>
                    <label for="card-subtitle-italic-${card.id}" class="flex items-center space-x-2 text-sm cursor-pointer"><input type="checkbox" id="card-subtitle-italic-${card.id}" class="card-subtitle-italic" ${card.isSubtitleItalic ? 'checked' : ''}><span>Subtítulo Itálico</span></label>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 border-t pt-4">
                    <div><label for="card-bg-color-${card.id}" class="text-sm font-medium">Cor de Fundo</label><div class="color-input-wrapper mt-1"><input type="color" id="card-bg-color-${card.id}" class="card-bg-color" value="${card.backgroundColor}" aria-label="Cor de fundo do card"><input type="text" class="card-bg-color-text w-full p-2 border rounded-md" value="${card.backgroundColor}" aria-label="Valor hexadecimal da cor de fundo do card"></div></div>
                    <div><label for="card-title-color-${card.id}" class="text-sm font-medium">Cor do Título</label><div class="color-input-wrapper mt-1"><input type="color" id="card-title-color-${card.id}" class="card-title-color" value="${card.titleColor}" aria-label="Cor do título do card"><input type="text" class="card-title-color-text w-full p-2 border rounded-md" value="${card.titleColor}" aria-label="Valor hexadecimal da cor do título do card"></div></div>
                    <div><label for="card-subtitle-color-${card.id}" class="text-sm font-medium">Cor do Subtítulo</label><div class="color-input-wrapper mt-1"><input type="color" id="card-subtitle-color-${card.id}" class="card-subtitle-color" value="${card.subtitleColor}" aria-label="Cor do subtítulo do card"><input type="text" class="card-subtitle-color-text w-full p-2 border rounded-md" value="${card.subtitleColor}" aria-label="Valor hexadecimal da cor do subtítulo do card"></div></div>
                    <div><label for="card-border-color-${card.id}" class="text-sm font-medium">Cor da Borda</label><div class="color-input-wrapper mt-1"><input type="color" id="card-border-color-${card.id}" class="card-border-color" value="${card.borderColor}" aria-label="Cor da borda do card"><input type="text" class="card-border-color-text w-full p-2 border rounded-md" value="${card.borderColor}" aria-label="Valor hexadecimal da cor da borda do card"></div></div>
                    <div><label for="card-border-width-${card.id}" class="text-sm font-medium">Largura da Borda (px)</label><input type="number" id="card-border-width-${card.id}" min="0" max="20" class="card-border-width w-full p-2 border rounded-md mt-1" value="${card.borderWidth}"></div>
                    <div><label for="card-border-radius-${card.id}" class="text-sm font-medium">Arredondamento (px)</label><input type="number" id="card-border-radius-${card.id}" min="0" max="30" class="card-border-radius w-full p-2 border rounded-md mt-1" value="${card.borderRadius}"></div>
                </div>
            `;
